...
describe('VizSelectorPanel', () => {
  ...
  test('Should match existing snapshot when given no initial visualization.', () => {
    expect(toJson(shallow(<VizSelectorPanel dataDir={testDataDir} />))).toMatchSnapshot();
  });

  Object.keys(VIZ_TYPE).forEach(viz => {
    test(`Should match existing snapshot when given initial viz of ${viz}.`, () => {
      expect(
        toJson(shallow(<VizSelectorPanel initialViz={viz as VIZ_TYPE} dataDir={testDataDir} />)),
      ).toMatchSnapshot();
    });
  });
  ...
  test('Should throw an error when given an invalid visualization option.', () => {
    ...
    expect(() =>
      shallow(<VizSelectorPanel initialViz={'Through the Looking Glass' as VIZ_TYPE} dataDir={testDataDir} />),
    ).toThrow();
  });
});
