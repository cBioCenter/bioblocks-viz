...
import { ContainerReducer } from '~bioblocks-viz~/reducer';
...
describe('ContainerReducer', () => {
  ...
  it('Should handle adding elements.', () => {
    ...
    let state = reducer(undefined, { type: 'BIOBLOCKS/INDICES_ADD', payload: 4 });
  ...
  it('Should handle adding multiple elements.', () => {
    const expectedState = Set<number>([4, 2, 42]);
    const reducer = ContainerReducer('indices');
    const state = reducer(undefined, { type: 'BIOBLOCKS/INDICES_ADD_MULTIPLE', payload: [4, 2, 42] });
    expect(state).toMatchObject(expectedState);
  });
  ...
  it('Should handle clearing elements.', () => {
    ...
    const reducer = ContainerReducer('indices');
    const state = reducer(undefined, { type: 'BIOBLOCKS/INDICES_SET', payload: [1, 2, 3] });
  ...
  it('Should handle removing elements.', () => {
    ...
    const reducer = ContainerReducer('nier');
    const state = reducer(undefined, { type: 'BIOBLOCKS/NIER_SET', payload: [1, 2, 3] });
    expect(state).not.toMatchObject(expectedState);
  ...
  it('Should handle setting elements.', () => {
    ...
    const state = reducer(undefined, { type: 'BIOBLOCKS/NIER_SET', payload: [1, 2, 3] });
    expect(state).toMatchObject(expectedState);
    expect(reducer(state, { type: 'BIOBLOCKS/NIER_SET', payload: [4] })).toMatchObject(Set([4]));
  });
});
