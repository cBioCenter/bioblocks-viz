import { AminoAcid, COUPLING_SCORE_SOURCE, CouplingContainer, ICouplingScore } from '~bioblocks-viz~/data';
...
describe('CouplingContainer', () => {
  ...
  describe('Amino acid helper', () => {
    ...
    it('Should allow the correct amino acid corresponding to an individual contact to be retrieved.', () => {
      ...
      const container = new CouplingContainer(contacts);
      expect(container.getAminoAcidOfContact(1)).toEqual(AminoAcid.Alanine);
      expect(container.getAminoAcidOfContact(2)).toEqual(AminoAcid.Asparagine);
    });
  });
